...
Let $L_{max}^{k}$ be the load on the machine with the largest load at step $k$, $w_k$ the weight of the item assigned at step $k$, $L^k_{min}$ is the load on the machine with the smallest load at step $k$, $L^k_{\tau} := L^k_{min} + w_k$.
Then
\begin{gather}
L^k_{\tau} = L_{max}^{k+1}.
\end{gather}
...
\begin{align*}
L_{min}^{j+1} &\geq L_{min}^{j} \\
w_{j+1} &\geq w_j.
\end{align*}
Then, using the induction hypothesis, we can make the following bound
\begin{gather*}
L_{\tau}^{j+1} = L_{\min}^{j+1} + w_{j+1} \geq L_{\min}^{j} + w_{j} = L_{\tau}^{j} \overset{\text{IH}}{=} L_{max}^{j+1}.
\end{gather*}
Now since truck $\tau$ was the only truck for which its load increased in the last step we find
\begin{gather*}
L_{\tau}^{j+1} \geq L_{max}^{j+1} \geq L_{t}^{t+1} \quad \forall t \in T.
\end{gather*}
By definition, $L_{max}^{j+2}$ is the largest load at the end of step $j+1$.
The above inequality implies that the largest load at time step $j+1$ is $L_{\tau}^{j+1}$, hence $L_{max}^{j+2} = L_{\tau}^{j+1}$.
\end{proof}
...
\begin{gather*}
L_{max}^{k} - w_{k} = L_{\tau}^{k-1} - w_{k} = L_{min}^{k-1} + w_{k-1} - w_{k} \leq  L_{min}^{k-1} \leq L_{min}^{k}.
\end{gather*}
