...
Let $L_{max}^{k}$ be the load on the machine with the largest load at step $k$, $w_k$ the weight of the item assigned at step $k$, $L^k_{min}$ is the load on the machine with the smallest load at step $k$, $L^k_{\sigma} := L^k_{min} + w_k$.
Then
\begin{gather}
